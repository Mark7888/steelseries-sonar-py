...
def StreamerMode(self):
    try:
        # check if streamer mode is enabled
        StreamerMode = requests.get(self.base_url + "/streamRedirections/isStreamMonitoringEnabled", verify=False)
    except:
...
class Sonar:
    ...
    def __init__(self, streamer_mode=None, app_data_path=None):
        ...
        if streamer_mode is None:
            ...
            self.streamer_mode = StreamerMode(self)
